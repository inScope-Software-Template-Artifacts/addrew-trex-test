...
import (
	...
	"github.com/openshift-online/addrewTrex/pkg/config"
...
func clone(_ *cobra.Command, _ []string) {
	...
	err := filepath.Walk(".", func(path string, info os.FileInfo, err error) error {
		...
		if strings.Contains(dest, "addrewTrex-service") {
			dest = strings.Replace(dest, "addrewTrex-service", strings.ToLower(provisionCfg.Name), -1)
		}
		...
		if info.IsDir() {
			...
			if strings.Contains(content, "github.com/openshift-online/addrewTrex") {
				...
				content = strings.Replace(content, "github.com/openshift-online/addrewTrex", replacement, -1)
			}
			...
			if strings.Contains(content, "addrewTrex") {
				glog.Infof("find/replace required for file: %s", path)
				content = strings.Replace(content, "addrewTrex", strings.ToLower(provisionCfg.Name), -1)
			}
			...
			if strings.Contains(content, "addrewTrex-service") {
				glog.Infof("find/replace required for file: %s", path)
				content = strings.Replace(content, "addrewTrex-service", strings.ToLower(provisionCfg.Name), -1)
			}
