...
![Trexxy](rhtap-addrewTrex-service_sm.png)
...
$ make db/login
    ...
    root@f076ddf94520:/# psql -h localhost -U addrewTrex-service addrewTrex
...
--------+------------+-------+---------------------
 public | dinosaurs  | table | addrewTrex-service
 public | events     | table | addrewTrex-service
 public | migrations | table | addrewTrex-service
(3 rows)
...
curl http://localhost:8000/api/addrewTrex/v1/dinosaurs | jq
...
```
{
  "kind": "Error",
  "id": "401",
  "href": "/api/addrewTrex/errors/401",
  "code": "API-401",
...
```
ocm get /api/addrewTrex/v1/dinosaurs
{
  "items": [],
  "kind": "DinosaurList",
  "page": 1,
...
ocm post /api/addrewTrex/v1/dinosaurs << EOF
{
    "species": "foo"
}
EOF
  ...
  "items": [
    {
      ...
      "href":"/api/addrewTrex/v1/dinosaurs/2XIENcJIi9t2eBblhWVCtWLdbDZ",
...
$ ocm login --token=${OCM_ACCESS_TOKEN} --url=https://addrewTrex-service.apps-crc.testing --insecure
...
- `cmd/addrewTrex-service/environments/service_types.go` : Add new service locator for the kind
- `cmd/addrewTrex-service/environments/types.go` : Add service locator and use `cmd/addrewTrex-service/environments/framework.go` to instantiate
- `cmd/addrewTrex-service/server/routes.go` : Add service routes (if missing)
...
Here's a reference MR for the same : https://github.com/openshift-online/addrewTrex/pull/25